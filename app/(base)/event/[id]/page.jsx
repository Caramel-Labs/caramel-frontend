...
export default async function Event({ params }) {
	...
	const event = await getData(
		`https://loop-backend.cyclic.app/events/${params.id}`
	);
	console.log(event);
	const {
		_id,
	...
	const people = ["kalana2001", "ravindu2001", "manujaya2001"];
	const props = {
		eventId: params.id,
		initialLikes: likes,
	};
	...
	return (
		<div className="h-screen mb-28">
			<Image
				...
				className=""
			></Image>
			<h1 className="text-white mx-4 mt-4 text-xl">{name}</h1>
			...
			<div className="mt-4 mx-4">
				<div className="flex items-center justify-between">
					<div className="flex">
						<div class="flex items-center justify-center -space-x-4 rtl:space-x-reverse pr-2">
							<img
						...
						<div className="flex items-center">
							<p className="text-white text-[8px]">
								{" "}
								{participants.length} already registered
							</p>
						</div>
					</div>
			...
			{/* Event description */}
			<p className="text-white text-xs mt-8 mx-4">{description}</p>
			...
			<div className="">
				...
				<div>
					<div class="flex mt-3">
						...
						<div class="w-3/4 p-4 pl-0">
							<p className="text-white text-xs font-bold">{venue}</p>
							<p className="text-white text-[8px]">Location</p>
						</div>
					</div>
				</div>
			...
			<div className="grid justify-items-center ">
				{tickets && <PaymentActionButton id={_id} />}
				{!tickets && <EventActionButton id={_id} />}
			</div>
		</div>
	);
}
